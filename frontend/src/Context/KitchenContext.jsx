...
const KitchenProvider = ({ children }) => {
  ...
  const fetchStatus = async () => {
    const res = await axios.get("https://food-court-seln.onrender.com/api/settings");
    if (res.data.success) {
  ...
  const toggleKitchen = async () => {
    const res = await axios.post("https://food-court-seln.onrender.com/api/settings/toggle-kitchen");
    if (res.data.success) {
      setKitchenOpen(res.data.kitchenOpen);
    }
  };
